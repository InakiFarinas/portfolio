...
export function OperatorPanel({ avatarSrc }: OperatorPanelProps) {
	...
	return (
		<aside className="flex flex-col gap-3 h-full overflow-y-auto py-3 px-3">
			...
			<div className="rounded-lg border border-[#1e2535] bg-[#0d1017] overflow-hidden">
				...
				<div className="flex items-center justify-between px-3 py-2 border-b border-[#1e2535]">
					...
					<div className="flex items-center gap-2">
						<WindowControls />
						<button
							className="md:hidden p-1.5 rounded hover:bg-[#161b27]"
							onClick={() => setExpanded((s) => !s)}
							aria-expanded={expanded}
							aria-label="Mostrar más"
						>
							<i
								className={`ti ${expanded ? "ti-chevron-up" : "ti-chevron-down"} text-[#8a9bbb]`}
							/>
						</button>
					</div>
				</div>
				...
				<div className="flex flex-col items-center gap-2 px-4 pt-4 pb-3 border-b border-[#1e2535]">
					<div className="relative">
						<div className="w-12 h-12 md:w-16 md:h-16 rounded-full overflow-hidden border-2 border-[#3d3272]">
							{avatarSrc ? (
								<img
									src={avatarSrc}
									alt={PROFILE.name}
								...
								<div className="w-full h-full bg-gradient-to-br from-[#3d3272] to-[#6b2d4e] flex items-center justify-center">
									<i className="ti ti-user text-xl md:text-2xl text-[#a78bfa]" />
								</div>
							)}
						</div>
						{/* Online dot */}
				...
				{/* Conditionally show stats and bio on small screens */}
				{expanded && (
					<>
						{/* Stats grid */}
						...
						{/* Bio */}
						<div className="rounded-lg border-t border-[#1e2535] bg-[#0d1017] overflow-hidden px-3 py-3">
							<p className="text-[11px] text-[#a8c5e8] font-mono leading-relaxed">
								Hola — soy Iñaki, frontend developer enfocado en interfaces
								reactivas y diseño de experiencias. Me gusta construir
								dashboards, interfaces minimalistas y mejorar la accesibilidad.
							</p>
			...
			{/* Activity Feed */}
			<div className="rounded-lg border border-[#1e2535] bg-[#0d1017] overflow-hidden flex-1 min-h-[120px] md:min-h-[200px]">
				<ActivityFeed />
			</div>
		</aside>
	);
